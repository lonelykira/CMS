...
public class AdminAttachmentAction extends AdminBaseAction {
	...
	public String photoManager(@RequestParam(value = "kindId") long kindId,
		...
		for (AttachmentVo attachment : attachmentList) {
			if (attachment.getType().equals(AttachmentConstant.Type.picture)
	...
	public String saveVideo(@RequestParam(value = "file") MultipartFile file) {
		JSONObject json = new JSONObject();
		try {
			String uploadPath = attachmentService.uploadVideo(file);
	...
	public String savePicture(@RequestParam(value = "file") MultipartFile file) {
		JSONObject json = new JSONObject();
		try {
			String uploadPath = attachmentService.uploadPicture(file);
			json.put("success", true);
			json.put("uploadPath", uploadPath);
		} catch (Exception e) {
			e.printStackTrace();
